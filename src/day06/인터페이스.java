...
//인터페이스의 static 메서드, 자바 8
	...
	// -> 구현 인터페이스가 100개 였다면? 100개의 클래스에 새로 추가될 추상메소드를 구현해야 할까..?이러한 문제점을 해결하기 위해 default method를 사용할 수 있다.
...
//	   private 메서드는 private, abstract, default 또는 static 메서드를 호출할 수 있다. private static은 static 및 static private 메서드만 호출 할 수 있다.
	...
	//5. 피드백: 인터페이스에서 정말 다 구현할 수 있을까? 인터페이스에서 구현을 하는 것과 추상 클래스에서 구현을 하는 것은 다르다.
	...
	//자바 8 이전에 인터페이스의 기본 메서드가 제공되지 않았던 시절 ->  중간에 추상클래스를 만들었다. 이 추상클래스를 확장하는 구현체에서는 필요한 메서드만 구현할 수 있도록 하는 일종의 편의성 제공 용도로 개발
	//이후 자바 8 이상에서 인터페이스의 기본 메서드가 제공됨에 따라 추상 클래스 없이도 개발 ->  구현체들은 상속에 대해 자유로워지게 되었다
	
	// 6.  함수형 인터페이스: 함수형 인터페이스는 인터페이스 안에 추상 메서드가 1개만 존재하는 경우를 말한다.
	
	// 7.강한 결합: A->B A의 내부에서 B객체 사용으로 결합, 느슨한 결합:  A -> 껍데기 I) - 구현 ->B : A내부에서 I객체 사용 -> B는 I)구현 으로 연결.  만약, B를 C로 바꾸려면, I)구현C로 바꾸면 된다.
